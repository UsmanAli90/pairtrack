...
type GoalRel = { id: number; title: string; pair_id: number }
type GoalUpdateJoinRow = {
  ...
  body: string | null
  created_at: string
  goals: GoalRel | GoalRel[] | null
}

const goalTitle = (g: GoalRel | GoalRel[] | null): string =>
  Array.isArray(g) ? (g[0]?.title ?? '(goal)') : (g?.title ?? '(goal)')
...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const loadUpdates = async (pid: number) => {
      ...
      const rows = (uRes.data ?? []) as unknown as GoalUpdateJoinRow[]
  ...
  const submitGoalCheckin = async () => {
    if (!selectedGoalId) {
      setErr('Pick one of your goals first.')
      return
    }
    const userRes = await supabase.auth.getUser()
    const user = userRes.data.user
      ...
      <div className="rounded-2xl border bg-white p-4 flex justify-between items-start">
        <div>
          ...
          <p className="text-gray-600">
            Partner: <span className="font-medium">{partner?.full_name || partner?.email || '—'}</span>
          </p>
        </div>
      </div>
        ...
        <div className="rounded-2xl border bg-white p-4">
          ...
          <div className="mt-4 border-t pt-4 space-y-2">
            ...
            <textarea
              ...
              className="w-full rounded-2xl border px-3 py-2"
            />
            <button onClick={addGoal} className="rounded-xl px-3 py-2 bg-gray-900 text-white">
              Add Goal
            </button>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="rounded-2xl border bg-white p-4">
          ...
          <div className="flex gap-2">
            ...
            />
            <button onClick={addComment} className="rounded-xl px-3 py-2 bg-gray-900 text-white">
              Send
            </button>
          </div>
            ...
            {comments.map((c) => (
              <li key={c.id} className="rounded-2xl border px-3 py-2">
                <div className="text-xs text-gray-500">
                  {nameOf(c.user_id)} • {new Date(c.created_at).toLocaleString()}
                </div>
...
function GoalRow({
  ...
  goal: Goal
  canEdit: boolean
  onChange: (id: number, patch: Partial<Goal>) => Promise<void>
}) {
  return (
      ...
      <div className="flex items-center gap-3 mt-2">
        <div className="flex-1">
          <input
            ...
            value={goal.progress}
            onChange={(e) =>
              canEdit ? onChange(goal.id, { progress: Number(e.target.value) }) : Promise.resolve()
            }
            className="w-full"
            disabled={!canEdit}
          />
        ...
        <select
          ...
          value={goal.status}
          onChange={(e) =>
            canEdit ? onChange(goal.id, { status: e.target.value as GoalStatus }) : Promise.resolve()
          }
          disabled={!canEdit}
        >
